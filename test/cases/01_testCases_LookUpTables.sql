...
-- 01.1.2 Bestehendes Genre finden und ID zurückgeben
CALL getOrCreateGenre('Thriller', NULL);
-- 01.1.3 Genre ohne Bezeichnung anlegen
...
-- 01.2.2 Anlage einer Sprache mit ungültigem ISO-Code-Format
CALL getOrCreateLanguage('Slowakisch', 'sk_SK', NULL);

-- 01.3 Länder
---------------------------
-- 01.3.1 Anlage eines weiteren Landes
CALL getOrCreateCountry('Scotland', 'GB', NULL);
-- 01.3.2 Anlage eines Eintrags mit zu langem ISO-Code
CALL getOrCreateCountry('Suisse', 'SUI', NULL);
-- 01.3.3 Anlage eines Eintrags mit ungültigem ISO-Code-Format
CALL getOrCreateCountry('Morocco', 'M9', NULL);

-- 01.4 Orte
---------------------------
-- 01.4.1 Anlage eines neuen Ortes
CALL getOrCreateLocation('34225', 'Baunatal', 'Deutschland', NULL);
