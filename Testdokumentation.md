...
Diese Testdokumentation gibt einen Überblick über die erstellten Testfälle und dokumentiert diese mit
einer kurzen Beschreibung, dem erwarteten Ergebnis und der verwendeten Prozedur. Die Testfälle sind dabei
chronologisch aufgebaut und folgenden dem Aufbau der Skripte zur Erstellung der Testdaten und sind
fachlich zusammengefasst.
Die am Ende aufgeführten End-to-End-Tests führen die einzelnen Testfälle fachlich zusammen und prüfen
exemplarisch die Geschäftslogik anhand des Ausleihprozesses.

## Prozeduren installieren

## Testfälle ausführen

Die Testfälle sind im Ordner `test/` hinterlegt. Sie können einzeln als `CALL`/`SELECT` oder als
komplettes Skript ausgeführt werden, z. B.

```bash
psql -U postgres -d iulendmybook -f test/06_testCases_EndToEnd.sql
```

Die notwendigen Prozeduren werden in der Regel automatisiert mit Initialisierung der Datenbank ausgeführt,
können bei Bedarf aus dem Ordner `init/procedures` nachinstalliert werden.

| Datei                             | Inhalt                                               |
|-----------------------------------|------------------------------------------------------|
| `01_testCases_LookUpTables.sql`   | LookUp-Tabellen (Genre, Sprache, Land, Ort)          |
| `02_testCases_BookMasterData.sql` | Buch-Stammdaten (Adresse, Verlag, Autor, Buch)       |
| `03_testCases_UserMasterData.sql` | Benutzer-Stammdaten (Account, Adresse, Rolle)        |
| `04_testCases_BookCopyData.sql`   | Buchexemplare anlegen, ändern und löschen            |
| `05_testCases_BookLoan.sql`       | Zeitslots, Abholoptionen, Ausleihe, Suche, Bewertung |
| `06_testCases_EndToEnd.sql`       | Vollständiger Ausleihzyklus für Abholung und Versand |
...
### 06 – End-to-End-Tests (`06_testCases_EndToEnd.sql`)

Dieser Testfall prüft den vollständigen Lebenszyklus des Ausleihprozesses für beide
Bereitstellungsarten **Abholung** (`PICK_UP`) und **Versand** (`SHIPPING`). Der Ablauf ist jeweils
identisch:

1. Selektion eines verfügbaren Exemplars mit definiertem Titel (`isBorrowable`)
2. Anlage des Ausleihvorgangs über `createBookLoan()`
3. Beendigung des Vorgangs über `returnBook()` zum Folgetag
4. Abgabe einer Bewertung über `createBookRating()`

Der Verlauf wird in der Historientabelle `loan_history` festgehalten. Am Ende der Transaktion wertet
eine Abfrage Statuswechsel, Bereitstellungsart, Abholslot bzw. Versandadresse, Rückgabedatum und
Bewertung aus.

Zur Wiederholbarkeit läuft das Skript in einer Transaktion und wird am Ende mit `ROLLBACK`
zurückgerollt. Die Testdaten bleiben dadurch unverändert.

| #    | Bezeichnung             | Beschreibung                                                                                         | Erwartetes Ergebnis                                                                                        | Prozedur                    |
|------|-------------------------|------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------|-----------------------------|
| 06.1 | Ausleihe per Abholung   | Verfügbares Exemplar zum Titel `Beneath the Quiet Bridge` für `clara.neumann@example.org` ausleihen  | Vorgang angelegt, Status `REQUESTED`, Abholslot aus `pickup_option` des Besitzers                          | `createBookLoan`            |
| 06.2 | Rückgabe nach Abholung  | Rückgabe zum Folgetag (`CURRENT_TIMESTAMP + 1 day`)                                                  | Status `RETURNED`, Rückgabedatum gesetzt, Eintrag in `loan_history`                                        | `returnBook`                |
| 06.3 | Bewertung nach Abholung | Bewertung mit Score `5` und Kommentar zum Verleiher                                                  | Bewertung gespeichert, NOTICE mit Rating-ID                                                                | `createBookRating`          |
| 06.4 | Ausleihe per Versand    | Verfügbares Exemplar zum Titel `Das Echo der stillen Stadt` für `jonas.reuter@example.org` ausleihen | Vorgang angelegt, Status `REQUESTED`, ohne Zeitslot, Versandadresse des Entleihers                         | `createBookLoan`            |
| 06.5 | Rückgabe nach Versand   | Rückgabe zum Folgetag                                                                                | Status `RETURNED`, Rückgabedatum gesetzt, Eintrag in `loan_history`                                        | `returnBook`                |
| 06.6 | Bewertung nach Versand  | Bewertung mit Score `3` und Kritik am verspäteten Versand                                            | Bewertung gespeichert, NOTICE mit Rating-ID                                                                | `createBookRating`          |
| 06.7 | Auswertung der Historie | Abfrage über `loan_history` inkl. Status, Fulfillment, Slot/Adresse, Rückgabe und Rating             | Ergebnisliste beider Vorgänge mit Statuswechsel `REQUESTED` → `RETURNED` sowie den abgegebenen Bewertungen | `SELECT` auf `loan_history` |