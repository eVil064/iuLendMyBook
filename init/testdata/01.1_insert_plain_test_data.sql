...
\COPY country (name, iso_code) FROM './resources/02_countries.csv' WITH (FORMAT CSV, HEADER true, DELIMITER ',');
...
\COPY language (name, iso_code) FROM './resources/04_languages.csv' WITH (FORMAT CSV, HEADER true, DELIMITER ',');

-- Zeitslots einfügen
-- Quelle: OpenAI. (2026). ChatGPT (Version GPT-5.6 Sol) .
-- https://chat.openai.com
-- Prompt:
\COPY timeslot (begin_time, end_time, day_of_week) FROM './resources/06_timeslots.csv' WITH (FORMAT CSV, HEADER true, DELIMITER ',');
